                      ...
                      onClick={async () => {
                        ...
                        try {
                          ...
                          const res = await fetch("/api/spotify/playlist", {
                            ...
                            body: JSON.stringify({
                              eventId: event.id,
                              playlistName: `${event.name} Playlist`,
                              description: `Music for ${event.name} generated by PartyPilot`,
                              songs: plan.playlist
                            })
                          });
...
function PlanSection({ title, items, children }: { title: string; items: string[]; children?: React.ReactNode }) {
  return (
    <BrutalCard className="print:shadow-none">
      <h3 className="text-3xl font-black">{title}</h3>
      <ul className="mt-5 space-y-3">
        {items.map((item) => (
          <li key={item} className="rounded-xl border-4 border-black bg-white p-4 font-bold">{item}</li>
        ))}
      </ul>
      {children}
    </BrutalCard>
